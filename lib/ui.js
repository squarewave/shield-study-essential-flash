...
function getFlashStatusForWindow(chromeGlobal) {
  const browser = chromeGlobal.gBrowser.selectedBrowser;
  return contentTask.spawn(browser, {}, () => content.document.documentFlashClassification);
}
...
function listenForPluginBrickInteractions(chromeGlobal) {
  ...
  pluginBrick.addEventListener('click', utils.storeClickedOnBrick);
}
...
exports.init = function () {
  // TODO: make a better every-so-often system
  setInterval(dataCollection.rollUpEvents, 1000 * 30);
