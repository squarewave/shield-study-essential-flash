...
function rollUpEvents () {
  try {
      ...
      if (event.type == "pageshow") {
        ...
        switch (event.type) {
          ...
          case "ProblemFixed":
            ...
            break;
          case "UserAction":
            if (event.kind != 'page-refreshed') {
              counts['user-action'][event.kind]++;
            }
    ...
    const payload = {
      ...
      geo: Services.prefs.getCharPref('browser.search.countryCode'),
      flashVersion: telemetryMetadata.flashVersion,
      datetime: Date.now(),
      profileAge: daysInExperiment + ss.storage.profileAgeInDays,
      daysInExperiment: daysInExperiment,
