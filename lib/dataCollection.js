...
let variation = null;
let experimentGroup = 'test-waiting';
let pingEvents = [];

function ingest(data) {
  pingEvents.push(data);
}
...
function getExperimentGroup() {
  ...
  const table = Services.prefs.getCharPref('urlclassifier.flashSubDocTable');
...
function rollUpEventsFromTimer() {
...
const init = Task.async(function* (which) {
  ...
  rollUpInterval = setInterval(rollUpEventsFromTimer, 1000 * 60 * 60 * 4);
